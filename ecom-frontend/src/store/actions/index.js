    ...
    (dispatch, getState) => {
        ...
        if (isQuantityExist) {
            const cartItem = {
                ...data,
                quantity: qty,
                // Ensure we have the image URL, fallback to product's image if not provided
                image: data.image || getProduct.image || 'no-image.png'
            };
...
export const getAllMessages = (queryString) => async (dispatch) => {
  try {
    dispatch({ type: "IS_FETCHING" });
    const { data } = await api.get(`/admin/messages?${queryString}`);
    dispatch({
      type: "FETCH_MESSAGES",
      payload: data.content,
      pageNumber: data.pageNumber,
...
export const deleteMessage = (messageId, toast) => async (dispatch) => {
  try {
